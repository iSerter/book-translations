# book-translations Development Guidelines

Auto-generated from all feature plans. Last updated: 2026-01-05

## Active Technologies
- Node.js >=22 <23 (per package.json) + `commander` (CLI), `better-sqlite3` (DB), `zod` (Validation) (003-import-book-json)
- SQLite (003-import-book-json)
- Node.js v24 + `commander` (CLI), `better-sqlite3` (DB), `zod` (Validation) (005-dhammapada-pali-support)
- SQLite (`src/data/book-translations.sqlite`) (005-dhammapada-pali-support)

- Node.js v24 (consistent with project) (002-book-export-formats)

## Project Structure

```text
src/
tests/
```

## Commands

# Add commands for Node.js v24 (consistent with project)

## Code Style

Node.js v24 (consistent with project): Follow standard conventions

## Recent Changes
- 005-dhammapada-pali-support: Added Node.js v24 + `commander` (CLI), `better-sqlite3` (DB), `zod` (Validation)
- 004-translate-chapter-titles: Added [if applicable, e.g., PostgreSQL, CoreData, files or N/A]
- 003-import-book-json: Added Node.js >=22 <23 (per package.json) + `commander` (CLI), `better-sqlite3` (DB), `zod` (Validation)


<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
